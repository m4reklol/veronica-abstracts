...
// ✅ RESPONSE HANDLER — GP Webpay callback (GET verze, funguje s GP adminem)
router.get("/response", async (req, res) => {
  console.log("📩 GP Webpay GET /response triggered");
  const {
    OPERATION,
    ORDERNUMBER,
    MERORDERNUM = "",
  ...
  try {
    ...
    console.error("❌ Error verifying digest:", err);
  ...
  if (paymentStatus === "paid") {
    ...
    console.log("🖼️ Označení produktů jako sold:", productIds);
    ...
    try {
      ...
      console.error("❌ Chyba při update produktů:", productErr);
    }
